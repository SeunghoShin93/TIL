...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            article.save()
            # hashtag
            for word in article.content.split():    # 공백 기준으로 리스트로 분할
                if word.startswith('#'):
                    hashtag, created = Hashtag.objects.get_or_create(content=word)  # word 랑 같은 해시태그를 찾는데 있으면 기존 객체 (.get), 없으면 새로운 객체 생성 (.create)
                    article.hashtags.add(hashtag) # created 를 사용하지 않았다면, hashtag[0] 으,로 작성
